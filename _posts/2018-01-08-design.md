...
Sur les 3 équipes qui se sont formées pour explorer les réalités de cette "start-up en 3h sans lignes de code", toute ont eu des membres qui ont ouvert par réflexe la console et l'inspecteur pour parcourir le code de la page web ou encore les perofmances ou le comportement de mémoire.
![](https://framapic.org/IAc3W1pYYoq3/TwlGbWH8C0qC)

Quelques personnes se sont lancées dans l'analyse du billet blog de l'entrepreneur pour tenter d'en comprendre les tenants et aboutissants de la démarche.
...
Certaines personnes se sont penchées sur un piste que je résumerai ainsi : 
l'existance d'un possible fichier texte contenant l'identifiant et mot de passe administrateur pour les [cron job](https://fr.wikipedia.org/wiki/Cron) dans `/var/www/html`

Les temps dédié à cette séance, 45 minutes, est une contrainte importante. Voyant les équipes prendre des pistes non justifiées (e.g. `/var/www/html`) ou investir un capital de temps lourd dans le reflex d'usage de la console et de l'inspecteur, je suggère d'exploiter une possibilité offerte par `traceroute` ou `curl` dans un terminal afin d'appronfondir leurs explorations. 
...
+ d'identifier les routeurs empruntés par les paquest de données
  + CDG − Paris (cdg ici)
...
On peut automatiser l'opération avec le langage de traitement de lignes [awk](https://fr.wikipedia.org/wiki/Awk) :
...
On peut ensuite lancer un navigateur Web directement vers la carte, ici avec OpenStreetMap (dans cette séance cela permet d'ouvrir vers un prochain cours sur la visualisation de données urbaines sur carte topographique) :
```
